<template>
  <BaseModal
    ...
    title="Gerenciar Estados (UF)"
    description="Cadastre, edite ou remova unidades federativas de apoio ao sistema"
    size="lg"
  >
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between gap-3 flex-wrap">
        <div class="relative flex-1 min-w-[200px]">
          ...
          <input
            v-model="searchTerm"
            type="text"
            placeholder="Filtrar por sigla ou nome do estado..."
        ...
        >
          <div class="flex items-center justify-between pb-2 border-b border-orange-200/60 dark:border-[#FC6714]/20">
            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
              ...
              <span>{{ editingId ? 'Editar Estado' : 'Novo Estado' }}</span>
            </h4>
            <button
              type="button"
              @click="closeForm"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            <div>
              ...
              <input
                ...
                type="text"
                placeholder="Ex: SP"
                class="w-full h-9 px-3 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 uppercase font-mono font-bold focus:ring-2 focus:ring-[#FC6714] outline-none"
              />
            </div>
        ...
        <div class="overflow-x-auto max-h-[380px]">
          <table class="w-full text-left border-collapse text-xs">
            <thead class="sticky top-0 bg-slate-100/90 dark:bg-[#03032E]/90 backdrop-blur-xs border-b border-slate-200 dark:border-[#14147A] text-slate-600 dark:text-slate-300 uppercase tracking-wider font-bold z-10">
              <tr>
                <th class="py-2.5 px-4 w-24">Sigla</th>
                <th class="py-2.5 px-4">Nome do Estado</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-[#14147A]/60">
              <tr v-if="loading">
                <td colspan="3" class="py-8 text-center text-slate-400">
                  <span class="inline-block animate-spin mr-1.5 text-[#FC6714]">⟳</span> Carregando estados...
                </td>
              </tr>
              <tr v-else-if="filteredStates.length === 0">
                <td colspan="3" class="py-8 text-center text-slate-400">
              ...
              >
                <td class="py-2.5 px-4 font-mono font-bold text-slate-900 dark:text-slate-100">
                  <span class="px-2 py-0.5 rounded bg-slate-200/70 dark:bg-slate-800 text-slate-800 dark:text-slate-200">
                    {{ st.code }}
                  </span>
                </td>
      ...
      >
        ...
        >
          <div class="flex items-center gap-2 text-red-800 dark:text-red-200 font-medium">
            ...
            <span>Confirmar exclusão de <strong>{{ stateToDelete.name }} ({{ stateToDelete.code }})</strong>?</span>
          </div>
          <div class="flex items-center gap-2">
            <button
              type="button"
              @click="stateToDelete = null"
...
const stateToDelete = ref<any | null>(null);
...
const filteredStates = computed(() => {
  if (!searchTerm.value) return states.value;
  const t = searchTerm.value.toLowerCase();
  return states.value.filter(
    s => s.code?.toLowerCase().includes(t) || s.name?.toLowerCase().includes(t)
  );
});
...
const saveState = async () => {
  if (!form.code.trim() || form.code.trim().length !== 2) {
    formError.value = 'A sigla do estado deve conter exatamente 2 caracteres.';
    return;
  }
  if (!form.name.trim()) {
    formError.value = 'O nome do estado é obrigatório.';
    return;
  }
  ...
  try {
    ...
    if (editingId.value) {
      await axios.put(`/api/v1/states/${editingId.value}`, payload);
      setFeedback('Estado atualizado com sucesso!', 'success');
    } else {
      await axios.post('/api/v1/states', payload);
      setFeedback('Estado cadastrado com sucesso!', 'success');
    }
    ...
    formError.value = err.response?.data?.message || 'Erro ao salvar estado.';
  } finally {
    saving.value = false;
  }
};

const confirmDelete = (st: any) => {
  stateToDelete.value = st;
};
...
const deleteState = async () => {
  ...
  deleting.value = true;
  try {
    await axios.delete(`/api/v1/states/${stateToDelete.value.id}`);
    setFeedback('Estado excluído com sucesso!', 'success');
    stateToDelete.value = null;
    await loadStates();
    emit('updated');
  } catch (err: any) {
    setFeedback(err.response?.data?.message || 'Erro ao excluir estado.', 'error');
  } finally {
