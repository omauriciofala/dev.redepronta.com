<template>
  <BaseModal
    ...
    description="Cadastre, edite ou remova bairros vinculados aos municípios"
    size="xl"
  >
    <div class="space-y-4">
      <!-- Barra Superior: Filtro por Cidade, Busca e Botão Novo -->
        ...
        <div class="flex items-center gap-2 flex-1 min-w-[280px]">
          ...
          <div class="w-48">
            <select
              ...
              class="w-full h-10 px-2.5 rounded-lg border border-slate-200 dark:border-[#14147A] bg-slate-50 dark:bg-[#03032E] text-slate-900 dark:text-slate-100 text-xs focus:ring-2 focus:ring-[#FC6714] outline-none truncate"
            >
          ...
          <div class="relative flex-1">
            ...
            <input
              v-model="searchTerm"
              type="text"
              placeholder="Buscar bairro por nome ou zona..."
        ...
        >
          <div class="flex items-center justify-between pb-2 border-b border-orange-200/60 dark:border-[#FC6714]/20">
            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
              ...
              <span>{{ editingId ? 'Editar Bairro' : 'Novo Bairro' }}</span>
            </h4>
            <button
              type="button"
              @click="closeForm"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            ...
            <div>
              <label class="block font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">
                Região / Zona (Opcional)
              </label>
              <input
                v-model="form.zone"
                type="text"
                placeholder="Ex: Centro, Sul, Norte, Leste"
                class="w-full h-9 px-3 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-[#FC6714] outline-none"
              />
            </div>
          </div>
        ...
        <div class="overflow-x-auto max-h-[380px]">
          <table class="w-full text-left border-collapse text-xs">
            <thead class="sticky top-0 bg-slate-100/90 dark:bg-[#03032E]/90 backdrop-blur-xs border-b border-slate-200 dark:border-[#14147A] text-slate-600 dark:text-slate-300 uppercase tracking-wider font-bold z-10">
              <tr>
                <th class="py-2.5 px-4">Nome do Bairro</th>
                <th class="py-2.5 px-4 w-44">Município</th>
                <th class="py-2.5 px-4 w-28">Zona</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-[#14147A]/60">
              <tr v-if="loading">
                <td colspan="4" class="py-8 text-center text-slate-400">
                  <span class="inline-block animate-spin mr-1.5 text-[#FC6714]">⟳</span> Carregando bairros...
                </td>
              </tr>
              <tr v-else-if="filteredNeighborhoods.length === 0">
                <td colspan="4" class="py-8 text-center text-slate-400">
                  Nenhum bairro cadastrado.
                </td>
              </tr>
              <tr
                v-for="nh in filteredNeighborhoods"
                :key="nh.id"
              ...
              >
                ...
                <td class="py-2.5 px-4 text-slate-700 dark:text-slate-300 font-medium">
                  {{ nh.city?.name || '-' }} <span v-if="nh.city?.state?.code" class="text-slate-400">({{ nh.city.state.code }})</span>
                </td>
                <td class="py-2.5 px-4 text-slate-500 dark:text-slate-400">
                  <span v-if="nh.zone" class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                    {{ nh.zone }}
                  </span>
      ...
      >
        ...
        >
          <div class="flex items-center gap-2 text-red-800 dark:text-red-200 font-medium">
            ...
            <span>Confirmar exclusão do bairro <strong>{{ neighborhoodToDelete.name }}</strong>?</span>
          </div>
          <div class="flex items-center gap-2">
            <button
              type="button"
              @click="neighborhoodToDelete = null"
...
const neighborhoodToDelete = ref<any | null>(null);
...
const filteredNeighborhoods = computed(() => {
  let list = neighborhoods.value;
  if (searchTerm.value) {
    const t = searchTerm.value.toLowerCase();
    list = list.filter(
      n => n.name?.toLowerCase().includes(t) || n.zone?.toLowerCase().includes(t) || n.city?.name?.toLowerCase().includes(t)
    );
  }
  return list;
});
...
const loadCities = async () => {
  try {
    const res = await axios.get('/api/v1/cities', { params: { all: 1 } });
...
const loadNeighborhoods = async () => {
  ...
  try {
    ...
    if (filterCityId.value) {
      params.city_id = filterCityId.value;
    }
    const res = await axios.get('/api/v1/neighborhoods', { params });
...
const openCreateForm = () => {
  ...
  form.city_id = filterCityId.value || '';
  form.name = '';
  form.zone = '';
  formError.value = '';
...
const openEditForm = (nh: any) => {
  editingId.value = nh.id;
  form.city_id = nh.city_id;
  form.name = nh.name;
  form.zone = nh.zone || '';
  formError.value = '';
  isFormOpen.value = true;
...
const saveNeighborhood = async () => {
  if (!form.city_id) {
    formError.value = 'Selecione o Município.';
    return;
  }
  if (!form.name.trim()) {
    formError.value = 'O nome do bairro é obrigatório.';
    return;
  }
  ...
  try {
    const payload = {
      ...
      name: form.name.trim(),
      zone: form.zone ? form.zone.trim() : null,
    };

    if (editingId.value) {
      await axios.put(`/api/v1/neighborhoods/${editingId.value}`, payload);
      setFeedback('Bairro atualizado com sucesso!', 'success');
    } else {
      await axios.post('/api/v1/neighborhoods', payload);
      setFeedback('Bairro cadastrado com sucesso!', 'success');
    }
    ...
    formError.value = err.response?.data?.message || 'Erro ao salvar bairro.';
  } finally {
    saving.value = false;
  }
};

const confirmDelete = (nh: any) => {
  neighborhoodToDelete.value = nh;
};
...
const deleteNeighborhood = async () => {
  ...
  deleting.value = true;
  try {
    await axios.delete(`/api/v1/neighborhoods/${neighborhoodToDelete.value.id}`);
    setFeedback('Bairro excluído com sucesso!', 'success');
    neighborhoodToDelete.value = null;
    await loadNeighborhoods();
    emit('updated');
  } catch (err: any) {
    setFeedback(err.response?.data?.message || 'Erro ao excluir bairro.', 'error');
  } finally {
